...
const FriendRowRequest: React.FC<FriendRowRequestProps> = ({
  friend_id,
  index,
}) => {
  ...
  const handleAccept = async () => {
    try {
      const response = await putData(`/trainer/accept_friend_request`, {
        index: index,
      });
  ...
  const handleDeny = async () => {
    try {
      const response = await putData(`/trainer/deny_friend_request`, {
        index: index,
      });
      console.log(response);
    } catch (e) {
  ...
  return (
    <div className="flex-col w-full bg-slate-800 p-2 items-center justify-between rounded-lg shadow-md">
