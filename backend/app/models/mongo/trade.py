...
class Trade:
    ...
    def __init__(
        self, trainer_id :str , friend_id:str, pkm_traded:int, pkm_received:int, trade_status : str
    ):
    ...
    def confirm_trade(data: dict) -> bool:
        if mongo.db is None:
            return False
        confirm : str = data.get("trade_status")
    ...
    def get_pending_trades(trainer_id : str)-> list | None:
        if mongo.db is None:
            return None
        pending_trades : list = list(mongo.db.trainers.find({"friend_id": trainer_id, "trade_status": "pending"}))
